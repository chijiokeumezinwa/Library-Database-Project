...
def create_app():
    ...
    # app.config['SQLALCHEMY_DATABASE_URI'] = f"mysql+pymysql://{db_user}:{db_password}@{db_host}/{db_name}" # Commented out
    # app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+mysqldb://{db_user}:{db_password}@localhost/{db_name}".format(
    #     db_user=db_user, db_password=db_password, db_name=db_name)
    
    # app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False # Commented out
    app.config['SECRET_KEY'] = os.environ.get('FLASK_SECRET_KEY')
    # db.init_app(app) # Commented out
    # migrate.init_app(app,db) # Commented out
    mail.init_app(app)

    # with app.app_context():
        # db.create_all() # Commented out - this will raise an error if db is not initialized

    @app.route('/users')
    def users():
        # from models import user # Will cause ImportError if models use db directly
        ...
        # from database import list_users # Will cause ImportError if database uses db directly
        # return {'users': list_users()}
        flash("Database is commented out, cannot list users.")
        return redirect(url_for('home')) # Redirect or return a placeholder
    ...
    @app.route('/')
    def home():
        # from models import user, Book # Will cause ImportError
        # current_user = None
        ...
        flash("Database is commented out, cannot display home content fully.")
    ...
    def login():
        # from database import list_users, verify, user # Will cause ImportError
        ...
        #         return redirect(url_for("login"))
        # #handle get request
        flash("Database is commented out, login functionality disabled.")
    ...
    def register():
        # from werkzeug.security import generate_password_hash # Keep if needed for other functionality
        # from models import user # Will cause ImportError
        # from sqlalchemy.exc import IntegrityError # Will cause ImportError
        ...
        flash("Database is commented out, registration functionality disabled.")
        return render_template("register.html")

    @app.route("/admin/")
    def admin():
        # from models import user, admin_request # Will cause ImportError
        ...
        flash("Database is commented out, admin functionality disabled.")
        return redirect(url_for('home'))
    ...
    def request_admin_request():
        ...
        flash("Database is commented out, admin request functionality disabled.")
        return redirect(url_for('home'))
    
    @app.route('/approve_admin_request/<int:request_id>')
    def approve_admin_request(request_id):
        # from models import admin_request, user # Will cause ImportError
        ...
        flash("Database is commented out, approve admin request functionality disabled.")
        return redirect(url_for('home'))
    ...
    def deny_admin_request(request_id):
        ...
        # return redirect(url_for('admin'))
        flash("Database is commented out, deny admin request functionality disabled.")
        return redirect(url_for('home'))

    @app.route("/request_book/<int:book_id>", methods=["POST"])
    def request_book(book_id):
        # from models import Book, Loan, user # Will cause ImportError
        ...
        flash("Database is commented out, book request functionality disabled.")
        return redirect(url_for('home'))
    ...
    def view_reservations():
        # from models import Reservation, Book, user # Will cause ImportError
        ...
        flash("Database is commented out, view reservations functionality disabled.")
        return redirect(url_for('home'))

    @app.route('/profile')
    def profile():
        # from models import user, Loan, Reservation, Book # Will cause ImportError
        # if 'current_user' not in session:
        ...
        flash("Database is commented out, profile functionality disabled.")
        return redirect(url_for('home'))
    ...
    def edit_profile():
        # from models import user # Will cause ImportError
        # from werkzeug.security import generate_password_hash # Keep if needed for other functionality
        # from sqlalchemy import and_ # Will cause ImportError
        ...
        flash("Database is commented out, edit profile functionality disabled.")
        return redirect(url_for('home'))
