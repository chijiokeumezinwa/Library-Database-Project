...
migrate = Migrate()
mail = Mail()
from models import user, admin_request, Category, Publisher, Book, Loan, Reservation
...
def create_app():
    ...
    # Database table creation (Keep this commented out; we use Flask-Migrate instead)
    ...
    def users():
        ...
        # from database import list_users # This import might be from an older structure.
        # It's generally better to use Flask-SQLAlchemy queries directly like below.
    ...
    def login():
        # Assuming database.py has list_users and verify functions that interact with the SQLAlchemy models
    ...
    def request_book(book_id):
        ...
        if book.copies_available > 0:
            ...
            # This part of the code suggests a reservation, but doesn't create a Reservation object.
            # If you have a separate reservation mechanism, this might be a placeholder message.
            # For a proper reservation, you'd uncomment the /reserve route and call it.
    ...
    def reserve_book(book_id):
        ...
        # Check if the user is an admin (Your original code seemed to restrict this to admins)
        # If regular users can reserve, remove this admin check.
        ...
        # Check if the book is already reserved by anyone (if a specific copy is reserved)
        # Or if the current user has already reserved it
        existing_reservation = Reservation.query.filter_by(book_id=book_id, user_id=current_user_id, status='Pending').first()
        if existing_reservation:
            flash(f'You have already reserved this book.', 'danger')
            return redirect(url_for('home'))

        # Create a reservation for the book
        reservation_date = datetime.utcnow().date()

        new_reservation = Reservation(user_id=current_user_id, book_id=book_id, reservation_date=reservation_date, status='Pending')
        db.session.add(new_reservation)

        # Decrement the available copy count of the book (This is for a real reservation, where it blocks a copy)
        book.copies_available -= 1
